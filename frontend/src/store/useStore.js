import { createContext, useContext, useReducer, useEffect, useState, useCallback, useRef, createElement } from 'react';
  ...
  switch (action.type) {
    ...
    case ACTIONS.LOAD_STATE: {
      ...
      return {
        ...
        bootstrapVersion: loaded.bootstrapVersion || BOOTSTRAP_VERSION,
        toasts:         [],
        confetti:       false,
        levelUpPending: null,
      };
...
export function StoreProvider({ children, username, defaultTheme }) {
  ...
  // ── Persist to localStorage + API (debounced) ───────────────────────────────
  const saveTimerRef = useRef(null);
  useEffect(() => {
    if (!synced) return; // wait until we've loaded from API before saving

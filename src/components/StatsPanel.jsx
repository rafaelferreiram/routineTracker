...
function RangeSelector({ ranges, activeIdx, onSelect, accent }) {
  return (
  ...
  return (
    <div className="rounded-3xl border overflow-hidden" style={{ background: '#0a0a0a', borderColor: '#1a1a1a' }}>
      ...
      <div className="flex items-center gap-4 px-5 pb-3">
        <div><p className="text-white font-bold text-lg leading-none">{avg}%</p><p className="text-[#4b5563] text-[10px] mt-0.5">Avg</p></div>
        <div className="w-px h-6 bg-[#1a1a1a]" />
        <div><p className="text-white font-bold text-lg leading-none">{peak}%</p><p className="text-[#4b5563] text-[10px] mt-0.5">Peak</p></div>
        <div className="w-px h-6 bg-[#1a1a1a]" />
        <div><p className="font-bold text-lg leading-none" style={{ color: accent }}>{perfect}</p><p className="text-[#4b5563] text-[10px] mt-0.5">Perfect days</p></div>
        {hover && (
          <>
            <div className="flex-1" />
            <div className="text-right">
              <p className="font-bold text-lg leading-none" style={{ color: accent }}>{hover.pct !== null ? `${hover.pct}%` : '—'}</p>
      ...
      <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full"
        ...
        <defs>
          <linearGradient id="cgrad" x1="0" y1={PAD.t} x2="0" y2={PAD.t + cH} gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor={accent} stopOpacity="0.35" />
            <stop offset="100%" stopColor={accent} stopOpacity="0.01" />
          </linearGradient>
        </defs>
        <ChartGrid />
        {segments.map((seg, i) => <path key={`f${i}`} d={buildSmoothPath(seg, true)} fill="url(#cgrad)" />)}
...
export default function StatsPanel() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Overall chart */}
      {habits.length > 0 && <CompletionChart habits={habits} accentColor={accentColor} />}
